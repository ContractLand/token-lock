...
contract TokenTimelock {
    ...
    constructor (IERC20 token, address beneficiary, address owner, uint256[] releaseTimes) public {
        ...
        for (uint i = 0; i < releaseTimes.length; i++) {
          require(releaseTimes[i] > block.timestamp, "TokenTimelock: release time is before current time");
        }
    ...
    function release() public {
        // solhint-disable-next-line not-rely-on-time
        require(_curRelease < _releaseTimes.length, "TokenTimelock: all limits has been released");
        require(block.timestamp >= _releaseTimes[_curRelease], "TokenTimelock: current time is before release time");
        ...
        require(amountTotal >= amountToRelease, "TokenTimelock: no tokens to release");
    ...
    function claim() public {
      ...
      require(amount > 0, "TokenTimelock: no tokens to claim");
